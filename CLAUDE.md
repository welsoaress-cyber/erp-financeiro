...
- **Etapa 56 entregue** (0095): `docs/60-api-integracoes.md` — API de consulta de cliente por CPF/CNPJ para integrações externas (Leveduca). Token por negócio (hash sha256, valor puro só na criação), Edge Function `api-consulta-cliente` (Verify JWT desligado), auditoria em `api_consultas` + relatório "Consultas à API". Configurações → Integrações via API. **Deploy pendente do proprietário**: migration 0095 pelo SQL Editor + Edge Function pelo painel (instruções no doc). Premissas assumidas (avisadas no doc, confirmar com a Leveduca): chave `endereco` sem acento, `numero`/`cep` sempre nulos (endereço é texto único no cadastro), grafia `cpf_cnpj` corrigida do typo do PDF deles.
