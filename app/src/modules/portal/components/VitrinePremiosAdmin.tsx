...
import { useFaixasAdmin, usePremiosAdmin, useSalvarFaixa, useSalvarPremio } from '../api'
...
export function VitrinePremiosAdmin({ negocioId, negocioSlug }: { negocioId: string; negocioSlug: string }) {
  ...
  const [janela, setJanela] = useState<{ tipo: 'faixa'; faixa?: IndicacaoFaixa } | { tipo: 'premio'; premio?: IndicacaoPremio } | null>(null)
  ...
  return (
    <Cartao className="p-0">
      ...
      <Modal aberto={janela !== null} aoFechar={() => setJanela(null)} titulo={janela?.tipo === 'faixa' ? (janela.faixa ? `Editar ${janela.faixa.faixa}ª faixa` : 'Nova faixa') : janela?.tipo === 'premio' ? (janela.premio ? 'Editar prêmio' : 'Novo prêmio') : ''}>
