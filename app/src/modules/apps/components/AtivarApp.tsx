...
import { formatarSaldo, type AppCatalogo, type ResumoCarteira } from '../tipos'
...
export function AtivarApp({ resumo, apps, planos, pessoas, aoConcluir }: { resumo: ResumoCarteira; apps: AppCatalogo[]; planos: Plano[]; pessoas: Pessoa[]; aoConcluir: () => void }) {
  ...
  const saldoDepois = app ? resumo.saldo - app.custo : resumo.saldo
  const insuficiente = app ? resumo.saldo < app.custo : false
  function aoEnviar(e: FormEvent) {
    ...
    setErro(null)
    ativar.mutate({ negocioId: resumo.negocio_id, pessoaId, appId: app.id, data, anuidade: a, diaVencimento: null, observacao: observacao.trim() || null }, { onSuccess: aoConcluir })
  }
  return (
    <form onSubmit={aoEnviar} className="space-y-4" noValidate>
      ...
      <Selecao rotulo="App" opcoes={[{ valor: '', rotulo: 'Selecione…' }, ...apps.filter((a) => a.ativo).map((a) => ({ valor: a.id, rotulo: `${a.nome} · ${formatarSaldo(a.custo, resumo.tipo_saldo)}` }))]} value={appId} onChange={(e) => setAppId(e.target.value)} />
      ...
      {app && (
        <div className={`rounded-md border p-3 text-sm ${insuficiente ? 'border-red-200 bg-red-50 text-red-800' : 'border-line bg-surface/60'}`}>
          <p>Consome <span className="font-semibold">{formatarSaldo(app.custo, resumo.tipo_saldo)}</span> · saldo depois: <span className="font-semibold">{formatarSaldo(saldoDepois, resumo.tipo_saldo)}</span></p>
          <p className="mt-1 text-xs opacity-80">Abre um contrato anual de {formatarMoeda(anuidade.trim() ? Number(anuidade.replace(',', '.')) || 0 : plano?.valor_tabela ?? 0)} (receita prevista) e lança a despesa do consumo na conta da carteira.</p>
