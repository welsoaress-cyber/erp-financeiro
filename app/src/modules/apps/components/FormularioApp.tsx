...
export function FormularioApp({ resumo, app, anuidadeAtual, aoConcluir }: { resumo: ResumoCarteira; app?: AppCatalogo; anuidadeAtual?: number; aoConcluir: () => void }) {
  const criar = useCriarApp()
  const atualizar = useAtualizarApp()
  const [nome, setNome] = useState(app?.nome ?? '')
  const [custo, setCusto] = useState(app ? String(app.custo) : '')
  ...
  const unidade = resumo.tipo_saldo === 'credito' ? 'créditos' : 'R$'
  function aoEnviar(e: FormEvent) {
    e.preventDefault()
    const c = Number(custo.replace(',', '.')); const a = Number(anuidade.replace(',', '.'))
    if (nome.trim().length === 0) { setErro('Informe o nome do app.'); return }
    if (custo.trim() === '' || Number.isNaN(c) || c < 0) { setErro(`Informe o custo em ${unidade}.`); return }
    if (!app && (anuidade.trim() === '' || Number.isNaN(a) || a < 0)) { setErro('Informe o valor da anuidade cobrada do cliente.'); return }
    setErro(null)
    if (app) atualizar.mutate({ id: app.id, nome: nome.trim(), custo: Math.round(c * 100) / 100, ativo }, { onSuccess: aoConcluir })
    else criar.mutate({ negocioId: resumo.negocio_id, nome: nome.trim(), custo: Math.round(c * 100) / 100, anuidade: Math.round(a * 100) / 100 }, { onSuccess: aoConcluir })
  }
  ...
  return (
    <form onSubmit={aoEnviar} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Campo rotulo={`Custo por ativação (${unidade})`} type="number" inputMode="decimal" step="0.01" min="0" value={custo} onChange={(e) => setCusto(e.target.value)} />
        {app
      ...
      {!app && <p className="text-xs text-ink-muted">Cria um plano anual no negócio com este nome e valor. A ativação abre um contrato desse plano.</p>}
