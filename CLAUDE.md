...
- **Etapa 55 (proposta, aguardando o sim):** `docs/59-compras.md` — módulo Compras com estrutura de time grande e tela de uma pessoa só (`exige_aprovacao` por negócio): requisição → aprovação → pedido → recebimento ("chegou tudo?") → nota → pagamento, com o recebimento disparando estoque/patrimônio/comodato/lançamento pelas funções existentes. Compras orquestra, nunca grava em lancamentos nem em estoque_itens.
