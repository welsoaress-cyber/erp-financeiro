...
for t in categorias lancamentos negocios pessoas contratos faturamento importacao recorrencias apps_saldo notificacoes notificacoes_envio portal portal_servnet recorrencia_fixa financeiro contratos_despesa saldo_inicial projecao_edicao projecao_contratos recorrencia_data periodicidades reancoragem excluir_cadeia importacao_opcional cartoes encargos disparos ftth estoque estoque_instalacoes os os_tecnico os_portal comodato pix_bloqueio bi ftth_ceo olt aceite excluir_pessoa natureza_categoria indicacao_presente patrimonio fechamento; do printf "%-12s " "$t"; (psql -q -d erp_test_b -f "supabase/tests/${t}_test.sql" 2>&1 || true) | grep -E "ERROR|^OK" | head -1; done
...
for m in 0014 0015 0013 0030 0016 0018 0021 0023 0024 0025 0026 0027 0028 0029 0031 0032 0033 0034 0035 0036 0037 0038 0040 0041 0042 0043 0044 0045 0046 0047 0048 0049 0050 0051 0052 0053 0054 0055 0056 0057 0058 0059 0060 0061 0062 0063 0064 0065 0066 0067 0068; do psql -q -d erp_test_c -v ON_ERROR_STOP=1 -1 -f supabase/migrations/2026090200${m}_*.sql 2>&1 | { grep -vE "NOTICE|DETAIL|drop cascades" || true; }; done
printf "%-12s " "producao"; psql -At -d erp_test_c -f supabase/tests/verificar_tudo.sql | grep TOTAL | awk -F'|' '{print ($2=="t"?"OK":"FALHOU") " (" $3 ")"}'
for t in contratos faturamento importacao recorrencias apps_saldo notificacoes notificacoes_envio portal portal_servnet recorrencia_fixa financeiro contratos_despesa saldo_inicial projecao_edicao projecao_contratos recorrencia_data periodicidades reancoragem excluir_cadeia importacao_opcional cartoes encargos disparos ftth estoque estoque_instalacoes os os_tecnico os_portal comodato pix_bloqueio bi ftth_ceo olt aceite excluir_pessoa natureza_categoria indicacao_presente patrimonio fechamento; do printf "%-12s " "prod:$t"; (psql -q -d erp_test_c -f "supabase/tests/${t}_test.sql" 2>&1 || true) | grep -E "ERROR|^OK" | head -1; done
