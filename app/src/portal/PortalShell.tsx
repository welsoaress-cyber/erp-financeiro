...
/** Área do cliente: exige sessão e vínculo com uma pessoa. Sem vínculo → /portal/vincular. */
export function PortalShell() {
  ...
  const cor = resumo.data.negocios[0]?.portal?.cor_primaria ?? '#1e3a8a'
        ...
        <header className="text-white" style={{ backgroundColor: cor }}>
          <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3">
            <div className="flex items-center gap-3">
              {resumo.data.negocios[0]?.portal?.logo_url && <img src={resumo.data.negocios[0].portal!.logo_url!} alt="" className="h-8 w-auto rounded bg-white/90 p-0.5" />}
              <div><p className="text-sm font-semibold leading-tight">{nomeNegocio}</p><p className="text-xs opacity-80">{resumo.data.pessoa.nome}</p></div>
            </div>
            <button type="button" onClick={() => sair()} className="rounded-md px-3 py-1.5 text-sm hover:bg-white/10">Sair</button>
          </div>
          <nav className="mx-auto max-w-4xl overflow-x-auto px-2">
            <ul className="flex gap-1 text-sm">
              {MENU.map((m) => (
                <li key={m.rota}><NavLink to={m.rota} end={m.fim} className={({ isActive }) => `block whitespace-nowrap rounded-t-md px-3 py-2 ${isActive ? 'bg-surface text-ink font-medium' : 'text-white/85 hover:bg-white/10'}`}>{m.rotulo}</NavLink></li>
              ))}
            </ul>
          </nav>
        </header>
        <main className="mx-auto max-w-4xl p-4 md:p-6"><ErrorBoundary><Outlet /></ErrorBoundary></main>
        <footer className="mx-auto max-w-4xl px-4 pb-6 text-center text-xs text-ink-muted">Portal do cliente · dúvidas: fale com {nomeNegocio}</footer>
      </div>
