-- =============================================================================
-- 0036 · Importação: CPF/CNPJ e plano opcionais
-- =============================================================================
-- Pedido do proprietário (clientes de servidor IPTV não têm CPF cadastrado):
-- - documento opcional: valida só quando presente; pessoa sem documento é
--   reaproveitada pelo NOME (case-insensitive) dentro da organização.
-- - plano opcional: quando vazio, usa o nome do negócio como plano.
-- Telefone segue obrigatório (lembrete de cobrança no WhatsApp).
-- =============================================================================
...
comment on function public.importar_clientes(uuid, jsonb, boolean, date) is
  'Importa clientes/planos/contratos de um CSV/planilha já lidos (jsonb). Documento e plano opcionais; telefone obrigatório. Simulação executa e desfaz.';
