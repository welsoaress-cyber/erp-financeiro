...
do $$ declare v_org uuid; v_neg uuid; v_cat uuid; v_item uuid; v_p uuid; v_plano uuid; v_ct uuid; r record; begin
  ...
  -- T2: comodato instalado aparece alocado ao cliente; saldo cai; situação abaixo do mínimo
  ...
  -- comodato não baixa o saldo (o bem continua da empresa); a saída por instalação baixa
  select * into r from public.vw_rel_estoque_itens where item_id = v_item;
  if r.quantidade_atual <> 2 or r.situacao <> 'ok' then raise exception 'T3 saldo=% situacao=%', r.quantidade_atual, r.situacao; end if;
  perform public.saida_estoque(v_item, 1, 'instalacao', current_date, v_p, v_ct, 'cabo 2');
  select * into r from public.vw_rel_estoque_itens where item_id = v_item;
  if r.quantidade_atual <> 1 or r.situacao <> 'abaixo_minimo' then raise exception 'T3b saldo=% situacao=%', r.quantidade_atual, r.situacao; end if;
end $$;
