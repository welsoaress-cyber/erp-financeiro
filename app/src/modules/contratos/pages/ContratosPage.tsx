...
import { useContratos, useCriarContrato, usePlanos, useReceitaRecorrente, useResultadoContratos } from '../api'
...
export function ContratosPage() {
  ...
  const carregando = contratos.isPending || planos.isPending || negocios.isPending || pessoas.isPending || resultado.isPending || mrr.isPending
  const erroCarga = contratos.error ?? planos.error ?? negocios.error ?? pessoas.error ?? resultado.error ?? mrr.error

  function fechar() { criar.reset(); setEdicao(null) }

  return (
    <>
      <CabecalhoPagina titulo="Contratos" descricao="Pessoas contratando planos dos seus negócios, com rentabilidade por contrato" acoes={<Botao onClick={() => setEdicao({ modo: 'novo' })} disabled={!temPlanos}>Novo contrato</Botao>} />
      ...
      <Modal aberto={edicao !== null} aoFechar={fechar} titulo={edicao?.modo === 'novo' ? 'Novo contrato' : contratoVisto ? `Contrato ${codigoContrato(contratoVisto)}` : 'Contrato'}>
        {edicao?.modo === 'novo' && (
          <FormularioContrato negocios={negocios.data ?? []} pessoas={pessoas.data ?? []} planos={planos.data ?? []} salvando={criar.isPending} erro={criar.error ? mensagemDeErro(criar.error) : null} aoSalvar={(d) => criar.mutate(d, { onSuccess: fechar })} aoCancelar={fechar} />
        )}
        {contratoVisto && (
          <DetalheContrato key={contratoVisto.id + contratoVisto.status} contrato={contratoVisto} nomes={{ negocio: nome.negocio.get(contratoVisto.negocio_id) ?? '—', pessoa: nome.pessoa.get(contratoVisto.pessoa_id) ?? '—', plano: nome.plano.get(contratoVisto.plano_id) ?? '—' }} resultado={resultadoDe.get(contratoVisto.id)} aoFechar={fechar} />
        )}
      </Modal>
    </>
  )
